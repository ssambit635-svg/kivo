 ...
 *
 * Vanilla port of the React Bits <ColorBends /> component (MIT). This repo
...
(() => {
  ...
  const DEFAULTS = {
    ...
    noise: 0.15,
    iterations: 2,
    intensity: 1.2, // >1.5 blows the overlapping bands out to white on paper
    bandWidth: 6,
  };
...
`;
  ...
  function createStage(stage, OGL, opts) {
    ...
    const program = new OGL.Program(gl, {
      ...
      blendFunc: { src: gl.ONE, dst: gl.ONE_MINUS_SRC_ALPHA },
      depthTest: false,
      depthWrite: false,
      uniforms: {
        uCanvas: { value: [1, 1] },
    ...
    const applyScroll = () => {
      ...
      u.uIntensity.value = opts.intensity + scroll * 0.35;
    ...
    if (reduced) {
      /* still life: one bent frame, no motion */
      renderOnce();
      return {
        reduced: true,
        setScroll: (p) => {
          scroll = scrollTarget = Math.min(1, Math.max(0, p));
          renderOnce();
        },
        destroy,
      };
