...
| Frontend | Complete - **"Verdant" refined-health redesign (2026)** at `/app/` — vanilla HTML/CSS/JS, zero build step. Warm paper + deep-jade palette with soft neutral shadows (no neon glow), generous spacing, splash · personalization onboarding · photo-card welcome (real imagery) · home with photo hero · profile page with **user profile-photo (PFP) upload** (canvas crop + monogram fallback) · 5-page bottom-nav shell (`theme.css` + `kivoux.js`) |
